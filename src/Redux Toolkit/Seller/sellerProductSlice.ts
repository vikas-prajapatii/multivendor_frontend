...
export const createProductsBulk = createAsyncThunk<
  Product[],
  { requests: any[]; jwt: string | null }
>(
  "sellerProduct/createProductsBulk",
  async ({ requests, jwt }, { rejectWithValue }) => {
    try {
      const response = await api.post<Product[]>(`${API_URL}/bulk`, requests, {
        headers: { Authorization: `Bearer ${jwt}` },
      });
      ...
      return rejectWithValue(error.response?.data || "Failed to bulk upload products");
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createProductsBulk.rejected, (state, action) => {
        state.loading = false;
        state.error = (action.payload as string) || "Failed to bulk upload products";
        state.productCreated = false;
      })
